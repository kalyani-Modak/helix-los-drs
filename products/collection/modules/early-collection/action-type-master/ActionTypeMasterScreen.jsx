...
import { useToast, HBox, HButtonBar, TitleBar, HLabel, HAgGrid, HAxiosService } from "@helix/component-library";



import { ActionTypeMasterAPI } from "../apiEndpoints.jsx";
import { buildActionTypeMasterColumnDefs } from "./actionTypeMasterGridColumns";
import "./actionTypeMaster.css";
...
const ActionTypeMasterScreen = () => {
  ...
  const handleFetch = useCallback(async () => {
    try {
      setLoading(true);
      const res  = await HAxiosService.GET(ActionTypeMasterAPI.ActionTypes(ScreenMenuId));
      const body = res?.data ?? {};
  ...
  const handleSave = useCallback(
    async ({ newRows = [], updatedRows = [], deletedRows = [] }) => {
      ...
      try {
        ...
        const res  = await HAxiosService.POST(ActionTypeMasterAPI.ActionTypes(ScreenMenuId), payload);
        const body = res?.data ?? {};
        ...
        toast.error(
          intl.formatMessage({id: "label.ActionTypeMaster.saveError",
  ...
  return (
    <>
      <HBox className="action-type-master-header-card">
        <HBox className="action-type-master-breadcrumb">
          <span className="action-type-master-bc-link">
            {intl.formatMessage({
              id: "label.ActionTypeMaster.breadcrumb.configuration",
              defaultMessage: "Configuration",
            })}
          </span>
          <span className="action-type-master-bc-sep" aria-hidden>›</span>
          <span className="action-type-master-bc-link">
            {intl.formatMessage({
          ...
          <span className="action-type-master-bc-sep" aria-hidden>›</span>
          <span className="action-type-master-bc-current">
            {intl.formatMessage({
        ...
        <TitleBar title="label.ActionTypeMaster.title" />

        <HLabel
          value={intl.formatMessage({
            id: "label.ActionTypeMaster.titleDesc",
            defaultMessage:
              "Categorise the types of actions (Outbound, Inbound, Field, Internal). Global Actions reference these.",
          })}
          align="left"
          colon={false}
        />
      ...
      <HBox className="action-type-master-grid-section">
        <HAgGrid
          ref={gridRef}
          key={intl.locale}
          rowData={rowData}
...
export default ActionTypeMasterScreen;
